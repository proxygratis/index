...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To Yudhy Network  Script Installer ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
echo -e "  This Will Quick Setup VPN Server On Your Server"
echo -e "  Auther : ${green}Yudhy DM ${NC}${YELLOW}(${NC} ${green}Project ${NC}${YELLOW})${NC}"
echo -e "  © Recode By Yudhy Project ${YELLOW}(${NC} 2023 ${YELLOW})${NC}"
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo ""
sleep 5
...
echo "$exp" >/usr/bin/e
versi="FREE"
echo "$versi" >/usr/bin/versi
# DETAIL ORDER
nama=$(cat /usr/bin/user)
versi=$(cat /usr/bin/versi)
exp=$(cat /usr/bin/e)
clear
#########################
# VPS Information
...
# Buat direktori xray
print_install "MAKE DIRECTORY XRAY"
    mkdir -p /etc/xray
...
function base_package() {
    clear
    ########
    print_install "INSTALLATION USE PACKAGE"
...
echo -e "   -----------------------------------------------  "
read -p "   Please select numbers 1 - 2 or Any Button( Random From xxx.free.yudhy.net) : " host
echo ""
if [[ $host == "1" ]]; then
...
function telegram() {
    ...
    MYIP=$(curl -sS ipv4.icanhazip.com)
    CHATID="1816225477"
    KEY="6322920332:AAFykYS80lOqpmSGhkE-NzzEB17l9LUct5M"
    TIME="10"
    ...
    =====================================
    (C) Copyright 2024 By Yudhy Network 2026
    =====================================
"
...
port 587
auth on
user yudhy.net@gmail.com
from yudhy.net@gmail.com
password perawan77 
logfile ~/.msmtp.log
EOF
